namespace Shapes2D {
	...
	public class Spawner : MonoBehaviour {
		...
		void OnGUI() {
			...
			offset.y += 30;
			GUI.Label(new Rect(offset.x, offset.y, 200, 20), "Shape Type (0 = random) (" + shapeType + ")");
			offset.y += 30;
			shapeType = (int) GUI.HorizontalSlider(new Rect(offset.x, offset.y, 100, 20), shapeType, 0, 15); 
			offset.y += 30;
				...
				if (spawned < count) {
					...
					if (shapeType == 0) {
						...
						if (shapeType >= 4) {
							...
							shape.settings.polygonPreset = (PolygonPreset) (shapeType - 3); 
						}
					}
					if (randomStyle) {
