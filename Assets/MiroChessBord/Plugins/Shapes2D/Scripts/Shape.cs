namespace Shapes2D {
    ...
    public class Shape : MonoBehaviour, IMaterialModifier {
        ...
        public class UserProps {
            ...
            public PolygonPreset polygonPreset {
                ...
                set {
                    if (value != PolygonPreset.Custom)
                        _vertices = Shape.GetVerticesForPreset(value);
            ...
            [SerializeField]
            private Vector2[] _vertices = GetVerticesForPreset(PolygonPreset.Pentagon);
            /// <summary>
            ...
            /// If you modify the returned vertex array directly rather than setting a new array, call settings.vertices = settings.vertices anyway so the shape knows to update the shader and values are sanitized.
            /// </summary>
            public Vector2[] vertices { 
                get { return _vertices; } 
                set { 
                    if (_vertices.Length < 3 || _vertices.Length > MaxPolygonVertices)
                    ...
                    for (int i = 0; i < _vertices.Length; i++) {
                        _vertices[i].x = Mathf.Clamp01(_vertices[i].x + 0.5f) - 0.5f;
                        _vertices[i].y = Mathf.Clamp01(_vertices[i].y + 0.5f) - 0.5f;
                    } 
        ...
        void ApplyShaderPropertiesToMaterial(Material material, bool disableBlending = false) {
            ...
            if (shaderSettings.shapeType == ShapeType.Rectangle) {
                ...
                material.SetFloat("_NumVerts", shaderSettings.numVerts);                
                    ...
                    material.SetVectorArray("_Verts", shaderSettings.vertices);
                #else
                    for (int i = 0; i < shaderSettings.numVerts; i++)
                        material.SetVector(ShaderVertexVarNames[i], shaderSettings.vertices[i]);
                #endif
        ...
        void ComputeShaderProperties() {
            ...
            if (settings.shapeType == ShapeType.Rectangle) {
                ...
                shaderSettings.numVerts = settings.vertices.Length;
                if (shaderSettings.vertices == null || shaderSettings.vertices.Length != shaderSettings.numVerts)
                    shaderSettings.vertices = new Vector4[shaderSettings.numVerts];
                // for the shader, each vertex is a Vector4 where zw is the endpoint of its line segment
                int j = shaderSettings.numVerts - 1;
                for (int i = 0; i < settings.vertices.Length; i++) {
                    Vector2 vi = new Vector2(settings.vertices[i].x * shaderSettings.xScale,
                            settings.vertices[i].y * shaderSettings.yScale);
                    Vector2 vj = new Vector2(settings.vertices[j].x * shaderSettings.xScale,
                            settings.vertices[j].y * shaderSettings.yScale);
                    shaderSettings.vertices[i] = new Vector4(vi.x, vi.y, vj.x, vj.y);
                    j = i;
                }
                ...
                if (shaderSettings.usePolygonMap) {
                    ...
                    if (settings.polyMapNeedsRegen) {
                        PolyMap.GeneratePolyMap(settings.vertices, shaderSettings.polyMap);
        ...
        /// You may want to look at settings.vertices instead.
        /// </summary>
        public Vector3[] GetPolygonWorldVertices() {
            if (settings.shapeType != ShapeType.Polygon)
                throw new System.NotImplementedException("GetPolygonWorldVertices() is only valid for polygons");
            Vector2 scale = GetScale();
            Vector3[] verts3D = new Vector3[settings.vertices.Length];
            for (int i = 0; i < settings.vertices.Length; i++) {
                Vector2 v2 = settings.vertices[i];
                v2.x *= scale.x;
                v2.y *= scale.y;
        ...
        /// <summary>
        /// If shapeType is Polygon, this sets its vertices from a list of world vertices.  
        ...
        public void SetPolygonWorldVertices(Vector3[] verts3D) {
            if (settings.shapeType != ShapeType.Polygon)
                throw new System.NotImplementedException("Shapes2D: SetPolygonWorldVertices() is only valid for polygons");
            if (verts3D.Length < 3 || verts3D.Length > MaxPolygonVertices)
                throw new System.InvalidOperationException("Shapes2D: SetPolygonWorldVertices() called with invalid number of vertices (must be between 3 and MaxPolygonVertices - currently " + MaxPolygonVertices + ")");
            Vector2 scale = GetScale();
