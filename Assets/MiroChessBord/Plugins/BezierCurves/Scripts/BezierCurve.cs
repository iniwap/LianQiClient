...
public class BezierCurve : MonoBehaviour {
	...
	{
		_length = 0;
		_LengthOnIndex.Clear ();
		_LengthOnIndex.Add (0);
		for (int i = 0; i < points.Length - 1; i++) {
			float SectionLength = ApproximateLength (points [i], points [i + 1], resolution);
			_length += SectionLength;
			_LengthOnIndex.Add (_length);
		}
		if (close) {
			float SectionLength = ApproximateLength (points [points.Length - 1], points [0], resolution);
			_length += SectionLength;
			_LengthOnIndex.Add (_length);
		}
	}
	...
	private List<float> _LengthOnIndex = new List<float>();
	...
	public Vector3 GetPointAtFloatIndex(float indexF)
	{
		if (_LengthOnIndex.Count == 0) {
			ComputeLength ();
		}

		for (int i = 0; i < _LengthOnIndex.Count; i++) {
			if (indexF >= (float)i && indexF < (float)(i + 1.0f)) {
				float len0 = _LengthOnIndex [i];
				float len1 = _LengthOnIndex [i + 1];
				float t01 = indexF - Mathf.Floor (indexF);
				float len = t01 * len1 + (1.0f - t01) * len0;
				float t = len / length;
				Vector3 pos = GetPointAt (t);
