...
public class AT摄像头控制 : MonoBehaviour {
	...
	public KeyCode _ZoomOut = KeyCode.Q;
	public KeyCode _ZoomIn = KeyCode.E;
	...
	void Update () {
        ...
        _观画摄像头.orthographicSize = Mathf.Clamp(
            _观画摄像头.orthographicSize, 0.01f, 100.0f);


		if (Input.GetKeyDown (_ZoomIn)) {
			开始放大 ();
		} else if (Input.GetKeyDown (_ZoomOut)) {
			开始缩小 ();
		}

		if (Input.GetKeyUp (_ZoomIn)) {
			停止缩放 ();
		} else if (Input.GetKeyUp (_ZoomOut)) {
			停止缩放 ();
		}
